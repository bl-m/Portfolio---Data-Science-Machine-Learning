/*
IBM DS Professional Certificate Program
Course 5/2 Lab 1
Basic SQL queries - DDL Statements
*/
...
/*
Q1: Were there any warnings loading data into the JOBS table? What can be done to resolve this?
...
/*
Q2: Did all rows from the source file load successfully in the DEPARTMENT table?
If not, are you able to figure out why not?
Answer: No, there were two DEPT_ID_DEP with the same value (5). Since DEPT_ID_DEP is
...
--Q3: Retrieve all employees in department 5 whose salary is between
--60000 and 70000 .
SELECT *
  FROM employees
...
--Q4B: Retrieve a list of employees ordered in descending order by department
...
--Q5A: For each department ID retrieve the number of employees in the department.
SELECT dep_id, COUNT(*)
  FROM employees
  GROUP BY dep_id;

--Q5B: For each department retrieve the number of employees in the department,
--and the average employees salary in the department.
SELECT dep_id, COUNT(emp_id), AVG(salary)
  FROM employees
  GROUP BY dep_id;

--Q5C: Label the computed columns in the result set of Query 5B as “NUM_EMPLOYEES”
-- and “AVG_SALARY”.
...
--Q5E: In Query 5D limit the result to departments with fewer than 4 employees.
SELECT dep_id, COUNT(emp_id) AS 'num_employees',
       AVG(salary) AS 'avg_salary'
  FROM employees
  ORDER BY avg_salary
